...
function createScheduleCard({ mission, customer, date, time, vehicle, payload, status }) {
  ...
  missionInfo.className = "md:col-span-2"
  const h3 = document.createElement("h3")
  h3.className = "font-semibold text-lg"
  h3.textContent = mission
  const p = document.createElement("p")
  p.className = "text-sm text-gray-400"
  p.textContent = customer
  missionInfo.appendChild(h3)
  missionInfo.appendChild(p)
  ...
  content.appendChild(missionInfo)
...
export function LaunchSchedule({ upcomingLaunches, pastMissions }) {
  ...
  h2.textContent = "Launch Schedule"
  const p = document.createElement("p")
  p.className = "text-lg text-gray-400 max-w-2xl mx-auto"
  p.textContent = "Stay updated with our upcoming missions and track our launch history."
  ...
  function renderCards(type) {
    ...
    const data = type === "upcoming" ? upcomingLaunches : pastMissions
